...
load(file = "./NO/02/input/dataset.RData")

## Load subjects

load(file = "./NO/01/output/subjects.RData")

## List of questionnaires

qnames = c("ICE-60", "CCPS", "NACC", "ETA-F", "ETA-U", "ULS", "MPS", "WTS")
...
for (i in 1:nquest) {
  ...
  items = read.table(file.path("./NO/02/input",fname), header = F, sep = "\t", encoding = "UTF-8")
...
# qdata = filter(df, name != "demo-0-no" & sid %in% subjects$sid)
# 
...
newcolnames = paste("ICE-60-no", questionnaires[["ICE-60"]]$code, sep = ".")
...
save(questionnaires, qdata, code_to_pl, code_to_en, code_to_no, 
     file = "./NO/02/output/dataset.RData")
